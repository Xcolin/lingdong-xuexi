...
当前 `server/` 使用 Flyway `V1` 至 `V22`，在既有组织、RBAC、系统任务、功能开关、数据字典、通用配置、平台账号 Web 会话、学生关系和学生登录安全链上，新增 `learningtask` 模块。该模块按领域对象、应用服务、MyBatis Mapper/XML 和 Web Controller 分层，任务定义、原始目标、标签、发布后学生实例及教师班级关系具有独立持久化边界；发布事务使用任务行锁、条件状态更新和任务学生唯一约束，批量发布通过独立 Bean 的 `REQUIRES_NEW` 方法建立逐项事务。

Spring MVC 继续在带 `@RequirePermission` 的接口前执行动态权限决策，用户级 `DENY` 优先于角色授权；学习任务应用服务进一步执行家庭主关系、机构组织子树、教师活动班级和学生本人对象范围。`web/` 已提供学习任务紧凑列表、筛选、编辑抽屉、单项/批量发布和班级关系配置；`miniapp/` 已提供学生本人任务入口、来源筛选、分页列表和只读详情。两个前端仍独立构建、独立会话和独立路由，不共享 Web 状态。

V22 未实现任务认领、打卡、审核、积分、消息和统计，也未完成对象存储与文件传输、模板执行、短信/微信/二维码认证及其他缓存域处理器。公开能力接口仅暴露客户端可见的非敏感开关摘要，后端功能开关仍是强制边界。
