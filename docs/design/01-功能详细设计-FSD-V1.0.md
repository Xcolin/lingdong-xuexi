...
| 学生登录 | 支持 4 位登录码、扫码、符合绑定条件后的微信登录；学生不得自主注册。 |
| 登录安全 | 学生登录码连续错误 5 次后需要图形验证码，连续错误 10 次锁定 15 分钟；二维码有效期 5 分钟。 |
| 设备管理 | 查看已登录设备、退出当前设备、一键下线；状态变更留痕。 |
| 密码与验证码 | 密码为 8 至 20 位字母加数字；验证码 6 位、5 分钟有效、1 分钟内最多发送 3 次。 |

### FSD-STU-01 学生账号与亲子关系

**主流程一（家庭）**：家长注册或登录 -> 创建学生 -> 自动生成登录码和二维码 -> 建立主家长关系。  
**主流程二（机构）**：机构管理员创建学生 -> 发送家长绑定邀请 -> 家长确认 -> 建立亲子与家校关联。

| 功能点 | 关键规则 |
|---|---|
| 学生创建 | 仅家长或机构管理员可创建；字段含学生姓名、年级、头像等。 |
| 主副家长 | 首次绑定为主家长；副家长默认查看与接收指定消息，无编辑、审核、奖励配置权限。 |
| 主家长变更 | 主家长解绑后，最早绑定的副家长自动升级；无副家长时家庭审核暂停，历史可查看。 |
| 机构邀请 | 机构不得强制绑定家长；邀请链接默认有效期 7 天，拒绝或过期不建立关系，可重新发送。 |
